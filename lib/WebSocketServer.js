...
WebSocketServer.prototype.setClientKey = function(fromKey, toKey) {
  var self = this;
  if(!(fromKey in self.clients)) {
    throw new Error(fromKey + ' is not present in the clients map');
  }
  self.clients[toKey] = self.clients[fromKey];
  self.clients[toKey].on('close', function() {
    delete self.clients[toKey];
  });

  console.log();
  console.log(Object.keys(self.clients));
  self.clients[fromKey].close();
  delete self.clients[fromKey];
  console.log(Object.keys(self.clients));
  console.log();
...
WebSocketServer.prototype.handleUpgrade = function(req, socket, u, cb) {
  ...
  var completeHybiUpgrade2 = function(protocol) {
    ...
    if(req.headers.authorization &&
      req.headers.authorization.split('Basic ').length === 2) {
      var basicAuth = req.headers.authorization.split('Basic ')[1];
      basicAuth = new Buffer(basicAuth, 'base64').toString();
      clientKey = basicAuth.split(':')[0];
    }
