    ...
    this.sender.close(code, data, mask, function(err) {
      ...
      if(self.closeReceived && self.isServer) {
        ...
        self.closeTimer = setTimeout(
          cleanupWebsocketResources.bind(self, true),
          closeTimeout
        );
      }
    });
...
WebSocket.prototype.terminate = function terminate() {
  ...
  if(!this.socket && this.readyState === WebSocket.CONNECTING) {
    return cleanupWebsocketResources.call(this, true);
  }
  this.readyState = WebSocket.CLOSING;
  try {
    this.socket.end();
  ...
  this.closeTimer = setTimeout(
    cleanupWebsocketResources.bind(this, true),
    closeTimeout
  );
};
...
function initAsClient(self, address, protocols, opts) {
  ...
  //
  // TODO: Factor out
  // var decoded = require('jsonwebtoken').verify(key, 'secret');
  //
...
//  var self = this;
  ...
  socket.on('end', cleanupWebsocketResources.bind(self));
  socket.on('close', cleanupWebsocketResources.bind(self));
  socket.on('error', cleanupWebsocketResources.bind(self));
