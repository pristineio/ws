...
var opcodes = {
  ...
  '1': {
    start: function(data) {
      ...
      if(firstLength < 126) {
        ...
        self.expect('header', 2, function(data) {
          opcodes['1'].getData.call(self, readUInt16BE.call(data, 0));
        });
      } else if(firstLength == 127) {
        self.expect('header', 8, function(data) {
          if(readUInt32BE.call(data, 0) != 0) {
            return self.error(
              'packets with length spanning more than 32 bit is currently ' +
                'not supported', 
              1008
            );
          }
    ...
    getData: function(length) {
      ...
      if(self.state.masked) {
        self.expect('header', 4, function(data) {
          var mask = data;
          ...
          function(err, buffer) {
            ...
            if(state.lastFragment) {
              ...
              if(!Validation.isValidUTF8(messageBuffer)) {
                return self.error('invalid utf8 sequence', 1007);
              }
  ...
  '2': {
    start: function(data) {
      ...
      if(firstLength < 126) {
        ...
        self.expect('header', 8, function(data) {
          if(readUInt32BE.call(data, 0) != 0) {
            return self.error('packets with length spanning more than 32 bit is ' + 
              'currently not supported', 1008);
          }
  ...
  '8': {
    start: function(data) {
      ...
      if(!self.state.lastFragment) {
        return self.error('fragmented close is not supported', 1002);
      }
      var firstLength = data[1] & 0x7f;
      if(firstLength < 126) {
    ...
    finish: function(mask, data) {
      ...
      this.messageHandlers.push(function() {
        if(data && data.length == 1) {
          return self.error('close packets with data must be at least two bytes long', 1002);
        }
        var code = data && data.length > 1 ? readUInt16BE.call(data, 0) : 1000;
        if(!(code in ErrorCodes)) {
          return self.error('invalid error code', 1002);
        }
        var message = '';
  ...
  '9': {
    start: function(data) {
      var self = this;
      if(!self.state.lastFragment) {
        return self.error('fragmented ping is not supported', 1002);
      }
      var firstLength = data[1] & 0x7f;
      if(firstLength < 126) {
  ...
  '10': {
    start: function(data) {
      var self = this;
      if(!self.state.lastFragment) {
        return self.error('fragmented pong is not supported', 1002);
      }
      var firstLength = data[1] & 0x7f;
      if(firstLength < 126) {
    ...
    getData: function(length) {
      ...
      if(this.state.masked) {
        this.expect('header', 4, function(data) {
          var mask = data;
          self.expect('data', length, function(data) {
        ...
        this.expect('data', length, function(data) {
