...
function readUInt16BE(data, start) {
  return (data[start]<<8) + data[start+1];
}

function readUInt32BE(data, start) {
  return (data[start]<<24) + (data[start+1]<<16) + (data[start+2]<<8) +
    data[start+3];
}
